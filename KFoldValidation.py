...
class KFoldValidationModel:
    ...
    def dumpFeatureVector(self,mask):
        underFetching = True
        # noOfFilesTrained = []
        for language in self.languages:
            X = []
            ...
            inputSize = 0
            flag = 0
            samples = AudioIO.getMaskedDumpSample(language,mask)
            print "Done"
            ct = 0
...
if __name__ == "__main__":
    ...
    for train_index, test_index in kf.split(X):
        K = KFoldValidationModel(AppConfig.languages, AppConfig.getTrainingDataSize())
        print "ITERATION:",ct+1
        ct+=1
        X_train, X_test = X[train_index], X[test_index]
        K.dumpFeatureVector(X_train)
        confusion_matrix.dumpConfusionMatrix()
        K.normFeature()
        K.selectFeature()
        K.train()
        K.binaryTrain()
        analysis = K.analyse(X_test)
