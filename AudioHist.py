...
languages = ["English", "Dutch", "Russian", "Spanish", "French"]
...
for i in range(len(languages)):
    dirList = [dir[0] for dir in os.walk(os.path.join(dir_path, "Dataset", languages[i]))]
    dirList = dirList[1:]
    for dirs in dirList:
        [mtFeatures, _, _] = audioFeatureExtraction.dirWavFeatureExtractionNoAveraging(os.path.join(dir_path, "Dataset", languages[i], dirs), 2.0, 1.0, 0.05, 0.025)
