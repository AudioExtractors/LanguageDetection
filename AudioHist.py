...
names = ['Zero Crossing Rate', 'Energy', 'Entropy of Energy', 'Spectral Centroid', 'Spectral Spread', 'Spectral Entropy', 'Spectral Flux',
...
Features = [[[] for x in range(len(names) * len(AvgFeatures))] for y in range(len(languages))]
...
for numOfAvgFeatures in range(len(AvgFeatures)):
    for numOfStats in range(len(names)):
        ...
        plt.title(AvgFeatures[numOfAvgFeatures] + " " + names[numOfStats])
        for i in range(len(languages)):
